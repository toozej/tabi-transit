...
func (s Service) RunOnce(ctx context.Context, limit int) (int, error) {
	...
	for _, delivery := range deliveries {
		...
		if quiet {
			...
			} else if err := s.Store.MarkRetry(ctx, delivery.ID, quietUntil, "quiet_hours"); err != nil {
				return 0, err
			}
			continue
		}
		...
		default:
			...
			} else if err := s.Store.MarkRetry(ctx, delivery.ID, next, "push_unavailable"); err != nil {
				return 0, err
			}
		}
	}
