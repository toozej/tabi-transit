...
psql "$database_url" -v ON_ERROR_STOP=1 -c 'CREATE TABLE IF NOT EXISTS public.schema_migrations (version text PRIMARY KEY, applied_at timestamptz NOT NULL DEFAULT now())'
for migration in "$migrations_dir"/*.up.sql; do
  version=$(basename "$migration")
  applied=$(psql "$database_url" -At -v ON_ERROR_STOP=1 -c "SELECT 1 FROM public.schema_migrations WHERE version = '$version'")
  if [[ "$applied" == "1" ]]; then
    continue
  fi
  printf 'Applying %s\n' "$version"
