...
history_schema=$(psql_in_container -At -v ON_ERROR_STOP=1 -c "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='history' AND table_name='vehicle_observations')")
[[ "$history_schema" == 't' ]] || { printf '%s\n' 'expected vehicle history schema' >&2; exit 1; }
psql_in_container -v ON_ERROR_STOP=1 -c "INSERT INTO history.vehicle_observations(source_id,public_id,source_vehicle_id,snapshot_id,mode,point,fetched_at,processed_at,freshness_status) SELECT 'fixture-rt','fixture:vehicle:history-old','history-old',id,'bus'::transit.mode,ST_SetSRID(ST_MakePoint(-122.67,45.52),4326)::geography,timestamptz '2026-06-27 12:00:00+00',timestamptz '2026-06-27 12:00:00+00','fresh'::realtime.freshness_status FROM realtime.snapshots WHERE source_id='fixture-rt' UNION ALL SELECT 'fixture-rt','fixture:vehicle:history-boundary','history-boundary',id,'bus'::transit.mode,ST_SetSRID(ST_MakePoint(-122.67,45.52),4326)::geography,timestamptz '2026-06-28 12:00:00+00',timestamptz '2026-06-28 12:00:00+00','fresh'::realtime.freshness_status FROM realtime.snapshots WHERE source_id='fixture-rt'"
psql_in_container -v ON_ERROR_STOP=1 -c "DELETE FROM history.vehicle_observations WHERE processed_at < timestamptz '2026-07-28 12:00:00+00' - interval '30 days'"
history_boundary=$(psql_in_container -At -v ON_ERROR_STOP=1 -c "SELECT public_id FROM history.vehicle_observations WHERE public_id LIKE 'fixture:vehicle:history-%' ORDER BY public_id")
[[ "$history_boundary" == 'fixture:vehicle:history-boundary' ]] || { printf 'unexpected retained history rows:\n%s\n' "$history_boundary" >&2; exit 1; }
trip_history_schema=$(psql_in_container -At -v ON_ERROR_STOP=1 -c "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='history' AND table_name='trip_update_observations')")
[[ "$trip_history_schema" == 't' ]] || { printf '%s\n' 'expected trip-update history schema' >&2; exit 1; }
psql_in_container -v ON_ERROR_STOP=1 -c "INSERT INTO history.trip_update_observations(source_id,snapshot_id,entity_id,trip_public_id,stop_sequence,arrival_delay_seconds,fetched_at,processed_at) SELECT 'fixture-rt',id,'history-old','fixture:trip:20-1',1,61,timestamptz '2026-06-27 12:00:00+00',timestamptz '2026-06-27 12:00:00+00' FROM realtime.snapshots WHERE source_id='fixture-rt' UNION ALL SELECT 'fixture-rt',id,'history-boundary','fixture:trip:20-1',1,-61,timestamptz '2026-06-28 12:00:00+00',timestamptz '2026-06-28 12:00:00+00' FROM realtime.snapshots WHERE source_id='fixture-rt'"
psql_in_container -v ON_ERROR_STOP=1 -c "DELETE FROM history.trip_update_observations WHERE processed_at < timestamptz '2026-07-28 12:00:00+00' - interval '30 days'"
trip_history_boundary=$(psql_in_container -At -v ON_ERROR_STOP=1 -c "SELECT entity_id FROM history.trip_update_observations WHERE entity_id LIKE 'history-%' ORDER BY entity_id")
[[ "$trip_history_boundary" == 'history-boundary' ]] || { printf 'unexpected retained trip-update rows:\n%s\n' "$trip_history_boundary" >&2; exit 1; }
