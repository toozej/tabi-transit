...
Last updated: 2026-07-23 (Phase 3 provider-independent foundation integrated)
...
Continue the Phase 2 arrival-estimation/trip-update writer slice, then favorites persistence/offline static artifact decision. Phase 3 may advance only through fixture-backed, provider-independent work until D-001 and D-004 are cleared; see `docs/BLOCKERS.md`. The physical development-build demonstration (restricted public Mapbox token, Android and iOS devices, and Maestro) is explicitly waived as a local-workstation blocker by the user, but remains unverified and required before any production-release claim.
