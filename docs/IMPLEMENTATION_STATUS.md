...
| Completed                | WP-11 planner adapter boundary, WP-15 non-host operations, WP-17 security/privacy guardrails                                      | Planner remains feature-disabled pending D-001; host/backup/metrics and legal evidence remain unverified.                                                       |
| Pending                  | WP-12, WP-18                                                                                                                      | Start only when their stated dependencies and approval gates are met.                                                                                           |
...
Continue the Phase 2 arrival-estimation/trip-update writer slice, then favorites persistence/offline static artifact decision. The physical development-build demonstration (restricted public Mapbox token, Android and iOS devices, and Maestro) is explicitly waived as a local-workstation blocker by the user, but remains unverified and required before any production-release claim.
