...
Last updated: 2026-07-30 (TriMet production composition, bounded history, API filtering, and accessible route/planner follow-ups are implemented)
...
| Completed / evidence-gated | WP-11 search/planner                                                                                    | The TriMet planner is normalized through an application mapper and composed only from validated server-side configuration. `/v1/journeys/plan` parses the provider-neutral contract and fails closed when the planner is not configured; the mobile remote repository calls only that normalized endpoint for opaque stop IDs. Mapbox search remains gated. |
...
| Completed / evidence-gated | WP-18 TriMet-derived presentation/history                                                               | Streetcar is normalized as a `streetcar` mode; Rose City-inspired presentation uses TriMet data only; bounded rider-visible vehicle history is retained and served for 30 days, and historical trip-update evidence is retained for evidence-only adherence classification. No direct Streetcar/PBOT/UmoIQ/Rose City dependency exists.                     |
| Blocked / evidence-gated   | Optional sources, production-host deployment, physical RNMapbox proof, mobile RNTL/Vitest harness proof | The documented Arrivals V2 boundary is fixture- and local smoke-tested with `TABI_TRIMET_*` configuration. The remaining source gate is Mapbox (D-004). Do not scrape.                                                                                                                                                                                      |
...
- D-011 Vitest and React Native Testing Library compatibility proof (the pinned RNTL dependency reaches React Native's untransformed Flow CommonJS entrypoint; an alternative Jest install is not available in the offline dependency cache).
- D-012 vehicle JSON versus GeoJSON hot-path contract selection. A local serialization comparison is recorded, but it is not native render evidence.
...
- Passed: fixture-only vertical-slice contract validation, OpenAPI structural validation and formatted generated-client drift check, public API/config Go tests, mobile Vitest (6 files/10 tests), Compose topology validation, and local compact-JSON versus GeoJSON payload construction. At 1,000/1,500/3,000 vehicles, JSON measured 97,954/147,479/296,054 bytes and GeoJSON measured 185,871/279,896/561,971 bytes; the corresponding local build/stringify runs were 0.32/0.46/0.83 ms and 0.73/1.42/1.41 ms. These are not native render-performance claims.
...
- Passed: `make test` now runs the root TypeScript suite, all deterministic
  mobile Vitest tests (18 files and 46 tests), and all Go tests. This closed a
...
- Passed: `/v1/vehicles` now validates and inclusively applies the documented
  WGS84 `bbox` filter. Request correlation IDs only propagate bounded safe
  tokens; missing or unsafe values are replaced with cryptographically random
  IDs. Both behaviors have HTTP coverage and do not log caller-controlled
  correlation content.
- Passed: the mobile route view loads validated route stops per direction and
  exposes their ordered, accessible stop links. Walking itinerary legs now
  offer an explicit external-maps handoff; Tabi does not share route
  coordinates until the rider invokes that action. The pure URL construction,
  route-stop boundary, and mobile fixture behavior are deterministic tests.

## Next integration milestone

TriMet production enablement is approved (D-001 resolved). The runtime planner
and its mobile remote boundary, plus bounded rider-visible vehicle-history
slices, are implemented. A deterministic maximum-page history-handler
benchmark is recorded locally; PostgreSQL, deployed HTTP/TLS, concurrency, and
mobile-render measurements remain needed for a release-scale claim. Historical
trip-update evidence is retained and classified without GPS inference; the
mobile UI deliberately says adherence is unavailable until a reviewed public
aggregate contract is added. Phase 3 Mapbox providers remain gated by D-004;
Phase 4 real notification permission, delivery, and receipts remain gated by
D-017 and D-018. Phase 6 host deployment remains gated by D-016. See
`docs/BLOCKERS.md`. The physical development-build demonstration (restricted
public Mapbox token, Android and iOS devices, and Maestro) is explicitly waived
as a local-workstation blocker by the user, but remains unverified and required
before any production-release claim. Vehicle direction filtering remains a
non-blocking implementation follow-up: current GTFS-Realtime data has no
normalized direction value, so it needs a reviewed static-trip enrichment
instead of silently accepting and ignoring the documented filter.

## Implemented vertical slices

| Slice                         | Delivered                                                                                                                                                                                                                                                 | Remaining evidence / deliberate boundary                                                                                               |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| TriMet journey planner        | Normalized TriMet mapper, validated startup composition, request/response handler, OpenAPI/client contract, fixture/HTTP tests, and mobile remote repository for opaque stop IDs. See ADR-0014.                                                           | Live provider verification remains opt-in; Mapbox place search is independent and still gated by D-004.                                |
| Rider-visible vehicle history | 30-day bounded storage, keyset history read, validation/ETag endpoint, generated client contract, accessible textual mobile timeline states, and a reproducible 500-observation handler benchmark. See ADR-0015 and `docs/benchmarks/vehicle-history.md`. | Add production PostgreSQL/query-plan, deployed HTTP/TLS/concurrency, and mobile-render measurements before a release-scale claim.      |
| Truthful adherence            | Historical trip-update delay/relationship evidence is retained for 30 days and classified early/on-time/late/canceled/unknown only from that evidence. See ADR-0016.                                                                                      | No public adherence aggregate is exposed yet; the mobile timeline explicitly reports it unavailable rather than inferring it from GPS. |
| Route and walking follow-up   | Direction-specific route-stop loading is validated and presented as ordered accessible links. Walking itinerary geometry can be handed to an external maps app only after an explicit rider action.                                                       | Native external-map behavior remains subject to the device verification gate; no coordinate is shared before the action.               |
